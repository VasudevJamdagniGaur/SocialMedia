import { POD_AI_TECH_EXPLORE_SLUGS, LS_AI_TECH_EXPLORE_STATS } from '../lib/podAiTechTrendingPersonalization';
...
function readLocalExploreStats() {
  ...
  return safeParseJson(localStorage.getItem(LS_AI_TECH_EXPLORE_STATS) || '{}', {});
}

function writeLocalExploreStats(stats) {
  if (typeof localStorage === 'undefined') return;
  try {
    localStorage.setItem(LS_AI_TECH_EXPLORE_STATS, JSON.stringify(stats));
  } catch {
    /* quota */
  }
}
...
/**
 * Bonus 0–55 added to effective trending rank per Explore category (mirrors sports explore weights).
 * @returns {Promise<Record<string, number>>}
 */
export async function getAiTechPersonalizationWeights() {
  ...
  const weights = {};
  if (total < 90) {
    for (const slug of POD_AI_TECH_EXPLORE_SLUGS) weights[slug] = 0;
    return weights;
  }
  for (const slug of POD_AI_TECH_EXPLORE_SLUGS) {
...
/**
 * @param {string} slug — one of POD_AI_TECH_EXPLORE_SLUGS
 */
export async function recordAiTechExploreDwell(slug, secondsDelta, visitInc) {
  if (!POD_AI_TECH_EXPLORE_SLUGS.includes(slug)) return;
