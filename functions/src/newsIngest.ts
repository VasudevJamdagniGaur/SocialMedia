...
async function tryNewsApi(key: string, q: string): Promise<IngestArticle[] | null> {
  ...
  try {
    ...
    logger.warn('[newsIngest] NewsAPI failed', { message: e instanceof Error ? e.message : String(e) });
    return null;
  }
}
