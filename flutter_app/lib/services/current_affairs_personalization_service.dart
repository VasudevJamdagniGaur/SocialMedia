...
const String lsCurrentAffairsExploreStats =
    'pod_current_affairs_explore_stats_v1';
...
Future<Map<String, dynamic>> _readLocalExploreStats() async {
  ...
  return _safeParseJson(
    prefs.getString(lsCurrentAffairsExploreStats) ?? '{}',
    {},
  );
}
...
Future<void> _writeLocalExploreStats(Map<String, dynamic> stats) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(lsCurrentAffairsExploreStats, jsonEncode(stats));
  } catch (_) {
    /* quota */
  }
}
...
Future<Map<String, int>> getCurrentAffairsPersonalizationWeights() async {
  final stats = await _readLocalExploreStats();
  final raw = <String, num>{};
  var total = 0.0;
  for (final slug in podCurrentAffairsExploreSlugs) {
...
Future<void> recordCurrentAffairsExploreDwell(
  String slug,
  num secondsDelta,
  num visitInc,
) async {
  if (!podCurrentAffairsExploreSlugs.contains(slug)) return;
