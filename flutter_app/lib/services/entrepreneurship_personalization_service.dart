...
const List<String> podEntrepreneurshipExploreSlugs = ['startups', 'founders'];

const String lsEntrepreneurshipExploreStats =
    'pod_entrepreneurship_explore_stats_v1';
...
Future<Map<String, dynamic>> _readLocalExploreStats() async {
  ...
  return _safeParseJson(
    prefs.getString(lsEntrepreneurshipExploreStats) ?? '{}',
    {},
  );
}
...
Future<void> _writeLocalExploreStats(Map<String, dynamic> stats) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(lsEntrepreneurshipExploreStats, jsonEncode(stats));
  } catch (_) {
    /* quota */
  }
}
...
Future<Map<String, int>> getEntrepreneurshipPersonalizationWeights() async {
  final stats = await _readLocalExploreStats();
  final raw = <String, num>{};
  var total = 0.0;
  for (final slug in podEntrepreneurshipExploreSlugs) {
...
Future<void> recordEntrepreneurshipExploreDwell(
  String slug,
  num secondsDelta,
  num visitInc,
) async {
  if (!podEntrepreneurshipExploreSlugs.contains(slug)) return;
