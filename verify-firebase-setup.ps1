...
if (Test-Path $googleServicesPath) {
    ...
    try {
        ...
        if ($packageName -eq "therapist.deite.app") {
            Write-Host "✅ Package name is correct: $packageName" -ForegroundColor Green
        } else {
            Write-Host "❌ WRONG package name: $packageName (should be therapist.deite.app)" -ForegroundColor Red
...
Write-Host "   - Package name matches: therapist.deite.app" -ForegroundColor Yellow
